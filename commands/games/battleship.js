...
module.exports = {
  ...
  aliases: ['battleships'],
    ...
    if (!devMode && !args.includes('skip')) {
      ...
      setTimeout(() => {warning(timer);}, 25000);

      function warning(number) {
        message.channel.send(`${number}!`);
        timer -= 1;
        if (timer == 0) {
          setTimeout(() => {question(-1);}, 1000);
          return null;
        } else {
    ...
    client.on('message', guessSubmission => {
      ...
      if (points[players.indexOf(guessSubmission.author.id)] == 5 || (guessSubmission.content.includes(config.prefix) && guessSubmission.content.includes(`giveup`) && fs.readFileSync('/home/runner/PeanutBotter/gamePosted.csv', 'utf-8').split(",")[serverIndex] == true)) {
